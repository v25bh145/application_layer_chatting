...
exports.c2sInstruction = function(request, respond, socket) {
    ...
    switch(instrHead[0]) {
        case "name":
            if(typeof(instrHead[1]) == "undefined" || !userModel.storeUser(instrHead[1], socket)) {
                respond = respondWrapper.setRespond("11", "10", "name-check", userModel.me(request.ip));
            } else {
                let welcome = "> name success, welcome " + instrHead[1] + " !";
                respond = respondWrapper.setRespond("00", "01", welcome, userModel.me(request.ip));
            }
            break;
        case "link-success":
            //TODO
        ...
        default:
            ...
            respond = respondWrapper.setRespond("10", "11", errorInfo, userModel.me(request.ip));
            break;
    }
    return respond;
}
exports.c2sMessage = function(request, respond, socket) {
    switch(request.bodyType) {
        ...
        case "text":
            //向其他人发送广播
            respond = respondWrapper.setRespond("00", "01", request.body, userModel.insteadOfMe(request.ip));
            break;
        case "file":
            //TODO
            break;

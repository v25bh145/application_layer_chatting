    ...
    switch(instrHead[0]) {
        case "name":
            if(typeof(instrHead[1]) == "undefined" || !userRepository.storeUser(instrHead[1], socket)) {
                respond = respondWrapper.setRespond(respond, "11", "10", "name-check", userRepository.me(request.ip));
            } else {
                let welcome = "> name success, welcome " + instrHead[1] + " !";
                respond = respondWrapper.setRespond(respond, "00", "01", welcome, userRepository.me(request.ip));
            }
            break;
        case "link-success":
            //TODO
        ...
        default:
            ...
            respond = respondWrapper.setRespond(respond, "10", "11", errorInfo, userRepository.me(request.ip));
            break;
    }
    return respond;
}
exports.c2sMessage = function(request, respond, socket) {
    switch(request.bodyType) {
        ...
        case "text":
            //向其他人发送广播
            respond = respondWrapper.setRespond(respond, "00", "01", userRepository.getUserName(request.ip) + ": " + request.body, userRepository.insteadOfMe(request.ip));
            break;
        case "file":
            //TODO
            break;
